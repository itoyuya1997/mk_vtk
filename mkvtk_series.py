...
pointdata1 = []
pointdatasetlines2 = ["{} {} {}\n".format("VECTORS","velocity","float")]
pointdata2 = []


###---write vtkfile---###
n = 1       #vtk file number
for i in range(0,len(dispx),a):
    pointdata1.clear()
    pointdata2.clear()
    ...
    celldata1.clear()
    celldata2.clear()
    for j in range(n9elem):
        celldata0 += ["{}\n".format(elements[j][2])]
        celldata1 += ["{}\n".format(strxz.iloc[i,j])]
        celldata2 += ["{}\n".format(strv.iloc[i,j])]
    ...
    with open(outputvtk,"w") as f:
        ...
        f.writelines(cells)         #"CELLS",n9elem,node_in_element+1
        f.writelines(cellset)       #define cells
        f.writelines(celltypes)     #"CELL_TYPES",n9elem
        ...
        ###---cell data---###
        f.writelines(celldatasetlines)        #"CELL_DATA",n9elem
